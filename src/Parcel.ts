...
export function handleMintParcel(_: MintCall): void {
  ...
  if (_.inputs.x1 < 0 && _.inputs.z1 < 0) {
    parcel.location = west.toString() + "W," + south.toString() + "S"
    parcel.save()}
  else if (_.inputs.x1 < 0 && _.inputs.z1 > 0) {
    parcel.location = west.toString() + "W," + north.toString() + "N"
    parcel.save()}
  else if (_.inputs.x2 > 0 && _.inputs.z1 < 0) {
    parcel.location = east.toString() + "E," + south.toString() + "S"
    parcel.save()}
  else  {
    parcel.location = east.toString() + "E," + north.toString() + "N"
    parcel.save()
  }
